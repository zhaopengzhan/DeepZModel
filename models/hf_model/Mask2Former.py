...
from utils.misc import calRunTimer, calRunTime
...
class Mask2Former(nn.Module):
    def __init__(self, model_id: str, in_chans: int, num_classes: int):
...
def test_build_model():
    with calRunTimer('模型构建'):
        model = Mask2Former(model_id=Mask2Former.get_model_ids()[0],
                            in_chans=4,
                            num_classes=32)
    # print(model)
    # pred_seg_map, loss = model(torch.randn(2, 4, 384, 384), label_map=torch.randint(0, 32, (2, 384, 384)))
    pred_seg_map, loss = model(torch.randn(2, 4, 384, 384))
